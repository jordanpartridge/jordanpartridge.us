...
<x-layouts.marketing>
    ...
    @volt('blog.index')
        <div class="relative flex flex-col w-full px-6 py-10 mx-auto lg:max-w-6xl sm:max-w-xl md:max-w-full sm:pb-16">
            ...
            <p class="w-full mt-2 text-base font-medium text-neutral-400 dark:text-slate-400 md:mt-2">Stay up to date with my various adventures</p>
                ...
                @foreach ($posts as $post)
                    <article class="flex space-x-8">
                        ...
                        <div class="w-7/12 mt-2">
                            ...
                            <p class="flex-grow text-lg font-light text-muted dark:text-gray-200">{{ $post->excerpt ?? substr(strip_tags($post->body), 0, 200) . (strlen(strip_tags($post->body)) > 200 ? '...' : '') }}</p>
